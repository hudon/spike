...
class Thalamus(module.Module):
    ...
    def connect(self, weight_GPi=-3, pstc_GPi=0.008, pstc_output=0.01, neurons_gate=40,
        ...
        for source, sink, conv, weight in self.bg.rules.get_rhs_routes():
            ...
            self.net.make(gname, neurons=neurons_gate, dimensions=1,
                encoders=[[1]], intercept=(gate_threshold, 1))
            ...
            self.net.make(cname, neurons=channel_N_per_D*vocab2.dimensions,
                dimensions=vocab2.dimensions)
            ...
            # N = channel_N_per_D*vocab2.dimensions
            self.net.connect(gname, cname, pstc=pstc_gate)
            # self.net.connect(gname, cname, encoders=-10, pstc=pstc_gate)
            # self.net.connect_neurons(gname, cname, weight_matrix=[[-10]]*N, pstc=pstc_gate)
