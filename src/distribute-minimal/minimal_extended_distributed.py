...
N_E = 600        # number of neurons in fifth population
N_F = 640        # number of neurons in sixth population
...
rate_E = 45, 80 # range of maximum firing rates for population E
rate_F = 20, 90 # range of maximum firing rates for population F
...
encoder_E = [random.choice([-1, 1]) for i in range(N_E)]
encoder_F = [random.choice([-1, 1]) for i in range(N_F)]
...
gain_E, bias_E = generate_gain_and_bias(N_E, -1, 1, rate_E[0], rate_E[1])
gain_F, bias_F = generate_gain_and_bias(N_F, -1, 1, rate_F[0], rate_F[1])
...
decoder_E = compute_decoder(encoder_E, gain_E, bias_E, function=function)
decoder_F = compute_decoder(encoder_F, gain_F, bias_F)
...
weights_DE=numpy.dot(decoder_D, [encoder_E])
weights_EF=numpy.dot(decoder_E, [encoder_F])
...
def ensemble_simulation_F(node_conn, ticker_conn):
    v_F = [0.0] * N_F       # voltage for population F     
    ref_F = [0.0] * N_F     # refractory period for population F
    output = 0.0            # the decoded output value from population F
    while True:

        t = ticker_conn.recv()
        if node_conn.poll():
            input_F = node_conn.recv()
        else:
            ticker_conn.send(t)
            continue
        ...
        total_F = [0] * N_F
        for j in range(N_F):
            total_F[j] = gain_F[j] * input_F[j] + bias_F[j]
        spikes_F = run_neurons(total_F, v_F, ref_F)
        # for each neuron in F that spikes, update our decoded value
        #  (also applying the same post-synaptic filter)
        output *= (1.0 - pstc_scale)
        for j,s in enumerate(spikes_F):
            if s:
                output += decoder_F[j][0] * pstc_scale
...
timerE_conn, ensembleE_conn = Pipe()
timerF_conn, ensembleF_conn = Pipe()
AB_conn, BA_conn = Pipe()
BC_conn, CB_conn = Pipe()
CD_conn, DC_conn = Pipe()
DE_conn, ED_conn = Pipe()
EF_conn, FE_conn = Pipe()

proc_A = Process(target=ensemble_simulation_A, args=(AB_conn,
    ensembleA_conn,), name="A ensemble")
...
proc_D = Process(target=ensemble_simulation_intermediate, args=(DC_conn, DE_conn,
    ensembleD_conn, N_D, N_E, gain_D, bias_D, weights_DE,), name="D ensemble")

proc_E = Process(target=ensemble_simulation_intermediate, args=(ED_conn, EF_conn,
    ensembleE_conn, N_E, N_F, gain_E, bias_E, weights_EF,), name="E ensemble")

proc_F = Process(target=ensemble_simulation_F, args=(FE_conn,
    ensembleF_conn,), name="F ensemble")
