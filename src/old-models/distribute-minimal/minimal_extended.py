...
v_E = [0.0] * N_E       # voltage for population E     
ref_E = [0.0] * N_E     # refractory period for population E
input_E = [0.0] * N_E   # input for population E

v_F = [0.0] * N_F       # voltage for population F     
ref_F = [0.0] * N_F     # refractory period for population F
input_F = [0.0] * N_F   # input for population F
...
while t < 10.0:
    ...
    for j in range(N_E):
        input_E[j] *= (1.0 - pstc_scale)
    for i,s in enumerate(spikes_D):
        if s:
            for j in range(N_E):
                input_E[j] += weights_DE[i][j] * pstc_scale

    total_E = [0] * N_E
    for j in range(N_E):
        total_E[j] = gain_E[j] * input_E[j] + bias_E[j]
    spikes_E = run_neurons(total_E, v_E, ref_E)
    for j in range(N_F):
        input_F[j] *= (1.0 - pstc_scale)
    for i,s in enumerate(spikes_E):
        if s:
            for j in range(N_F):
                input_F[j] += weights_EF[i][j] * pstc_scale
